...
class GenreDataset(Dataset):
    ...
    def __init__(self, sentences: List[str], labels: List[int], tokenizer: RobertaTokenizer, max_len: int = 128):
    ...
    @staticmethod
    def load_manually(file_dir: str, tokenizer: RobertaTokenizer, max_len: int) -> "GenreDataset":
        sentences = []
        labels = []
...
def create_model(scope: Dict[str, Any]) -> Tuple[RobertaForSequenceClassification, RobertaTokenizer]:
    device = scope["devices"][0]
    model_name = "roberta-" + scope["model"]
    config = RobertaConfig.from_pretrained(model_name)
    config.num_labels = 5

    tokenizer = RobertaTokenizer.from_pretrained(model_name)
    model = RobertaForSequenceClassification.from_pretrained(model_name, config=config) if not scope[
