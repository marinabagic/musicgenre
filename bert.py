...
class GenreDataset(Dataset):
    ...
    def __init__(self, sentences: List[str], labels: List[int], tokenizer: BertTokenizer, max_len: int = 128):
    ...
    @staticmethod
    def load_manually(file_dir: str, tokenizer: BertTokenizer, max_len: int) -> "GenreDataset":
        sentences = []
        labels = []
...
def create_model(scope: Dict[str, Any]) -> Tuple[BertForSequenceClassification, BertTokenizer]:
    device = scope["devices"][0]
    model_name = "bert-" + scope["model"] + "-uncased"
    config = BertConfig.from_pretrained(model_name)
    config.num_labels = 5

    tokenizer = BertTokenizer.from_pretrained(model_name)
    model = BertForSequenceClassification.from_pretrained(model_name, config=config) if not scope[
